...
**Usa SIEMPRE la plantilla del kit**: `${CLAUDE_PLUGIN_ROOT}/skills/kit-propuesta-pdf/references/plantilla-propuesta.html`. Léela y REESCRIBE el archivo de trabajo completo con Write sustituyendo los `{{PLACEHOLDERS}}` con los datos reales (no hagas find/replace: hay placeholders parecidos y quedarían textos duplicados); borra los bloques que no apliquen y descomenta los que sí (opción 3, página 2). Con UNA sola opción: usa clase `una` y QUITA el badge "Nuestra recomendación" (sin comparación no tiene sentido). NO rediseñes desde cero ni cambies tipografías, colores o estructura: la plantilla ES la marca de la red (acento dorado #c98a2d, carta, aire, cero saturación). Contenido por sección:
